...
- **Kun portal:** endepunktene krever portal-scope pluss tilgangsstyring-les for partyen. `includeMps` er ikke eksponert — maskinportenschema-hendelser forblir skjult.
- **Rollematrise:** hva kalleren får se avgjøres av deres effektive roller for partyen (løst opp gjennom connection-spørringen): tilgangsstyrere ser assignment-delen, klientadministratorer delegation-delen, og hovedadministratorer alt. Hver spørring — både hendelser og filterverdier — begrenses til de synlige typene. Ber man bare om typer utenfor sitt sett, returneres en tom side; en kaller uten matriserolle får 403. Matrisen er en kodetabell (`ActivityLogRoleMatrix`) ment å utvides etter hvert som loggen får flere datapunkter.
