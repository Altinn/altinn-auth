...
public class ClientDelegationService(AppDbContext db, IOptions<CoreAppsettings> appsettings) : IClientDelegationService
{
    ...
    public async Task<Result<List<ClientDto>>> GetClients(Guid partyUuid, List<string>? roles, List<string>? packages, CancellationToken cancellationToken = default)
    {
        ...
        var query = await db.Assignments
            ...
            .WhereIf(packageFilter.Count > 0, x => (x.RolePackage != null && (x.RolePackageEntityVariantId == null || x.RolePackageEntityVariantId == x.From.VariantId) && packageFilter.Contains(x.RolePackage.Id)) || (x.AssignmentPackage != null && packageFilter.Contains(x.AssignmentPackage.Id)))
            .GroupBy(x => x.From.Id)
            .ToListAsync(cancellationToken);
